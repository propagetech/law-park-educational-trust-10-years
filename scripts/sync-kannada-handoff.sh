#!/usr/bin/env bash
# Copy the Kannada film handoff page into public/ for the static site.
#
...
# by scripts/build-asset-drop.sh, so that:
#
...
rm -rf "$DEST" "$ROOT/public/anniversary-video-research"
mkdir -p "$DEST"
...
echo "Synced the handoff page to public/anniversary-video-production/kannada ($(du -sh "$DEST" | awk '{print $1}'))"
