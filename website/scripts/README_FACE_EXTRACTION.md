...
Run them from the `website/` directory. They resolve `public/images` relative to
the working directory, so running them from the repo root will not find the images.
