#!/usr/bin/env python3
"""
Extract candid photos of individual people from gallery images using MediaPipe.

This is an alternative to extract_faces.py that uses MediaPipe instead of face_recognition.
MediaPipe is easier to install and works cross-platform.
...
try:
    ...
    print("Run: pip install mediapipe opencv-python pillow numpy")
    sys.exit(1)


class FaceExtractorMediaPipe:
    def __init__(
        self,
        ...
        """
        Initialize the face extractor using MediaPipe.

        Args:
            input_dir: Directory containing source images
            output_dir: Directory to save extracted faces
            min_face_size: Minimum face size in pixels (width or height)
            padding: Padding around face as fraction (0.2 = 20%)
    ...
    def detect_faces(self, image: np.ndarray) -> List[Tuple[int, int, int, int]]:
        ...
        Returns:
            List of (x_min, y_min, x_max, y_max) face bounding boxes
        """
    ...
    def extract_face(
        ...
        Args:
            ...
            face_location: (x_min, y_min, x_max, y_max) face coordinates
    ...
    def process_all_images(self) -> dict:
        ...
        """
        print(f"\n{'='*60}")
        print("Face Extraction Started (MediaPipe)")
        print(f"{'='*60}")
        print(f"Input directory: {self.input_dir}")
        print(f"Output directory: {self.output_dir}")
        print(f"Min face size: {self.min_face_size}px")
        print(f"Padding: {self.padding * 100}%")
        print(f"{'='*60}\n")
...
def main():
    ...
    # Create extractor
    extractor = FaceExtractorMediaPipe(
        input_dir=input_dir,
